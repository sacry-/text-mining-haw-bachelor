...
\section{Vector Space Model (VSM)}
  ...
  \subsection{Enhancing the Vector Space Model}
    ...
    In figure \ref{pca} we find the \emph{n} most likely components of a document we will decompose an original document matrix $X$ by factoring out $P$ principal components. These components capture the variance throughout each document individually and reduce the dimensions down to a specified \emph{k}. The factorization then tries to solve for a principal component that bestly resembles the original data. If we do this with SVD we also get a ranking of these principal components by impact on the entire collection. Primarily \emph{PCA} can be described as a dimensionality reduction technique that resembles the original structure well. It can be used for visualization, reducing down $n + 3$ dimensions to 2D or 3D. See more in \cite{PCA2009}.\\
    The whole theoretical concept stems from least square projections to project the data into a plane for each dimension. Least square projections are useful for linear combinations of vectors that do not intersect. It is then possible to project data onto a line or a plane that can by some error value be part of a combined dimension. As with the \emph{SVD} we find several components that resemble the data without parts of the noisy variance.
...
\section{Clustering algorithms}
  ...
  \subsection{Evaluation}
    ...
    \paragraph{Internal measures}
      ...
      \begin{enumerate}
        \item \emph{Silhouette coefficient} - \cite{Nothing}
        \item \emph{Davies–Bouldin index} - \cite{Nothing}
        \item \emph{Dunn index} - \cite{Nothing}
      \end{enumerate}
